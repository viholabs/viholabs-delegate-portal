...
export default function HoldedLogBlock() {
  const [data, setData] = useState<HoldedLogResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        setLoading(true);
        const res = await fetch("/api/control-room/holded-log", {
          cache: "no-store",
        });
        const json = (await res.json()) as HoldedLogResponse;
        if (!cancelled) setData(json);
      } catch {
        if (!cancelled) {
          setData({
            ok: false,
            error: "No s’ha pogut carregar el log de Holded.",
          });
        }
      } finally {
  ...
  return (
    <div style={card}>
      ...
      <div style={title}>Holded Log</div>
          ...
          {items.slice(0, 4).map((item) => (
            <div key={item.id} style={row}>
              <div style={rowTop}>
                ...
                <span style={rowDate}>{fmtDate(item.finished_at || item.started_at)}</span>
              </div>

              <div style={metrics}>
                <span>imported {item.imported ?? 0}</span>
                <span>skipped {item.skipped ?? 0}</span>
